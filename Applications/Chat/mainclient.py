...
while True:
    ...
    try:
        ...
        while True:
            ...
            username_header = myself_socket.recv(HEADER_LENGTH)
            ...
            username = myself_socket.recv(username_length).decode('utf-8')

            # Now do the same for message (as we received username, we received whole message, there's no need to check if it has any length)
            message_header = myself_socket.recv(HEADER_LENGTH)
            message_length = int(message_header.decode('utf-8').strip())
            message = myself_socket.recv(message_length).decode('utf-8')
